...
- 13:00:00 前收到每日缓存提示时，等待至当天 **13:00:00**，到点先额外刷新一次主页并取得有效 Token，再从第一顺位提交；等待期间保留提前预热。整点刷新只触发一次，后续缓存提示按 `request_interval` 重试，不强制等待 800ms。截止时间仍优先。
- 满额按间隔继续重试；网络或服务异常按 1、2、4、8、12 秒退避，并尊重 `Retry-After`。
- 已取得受理号时继续查询同一请求，结果未知时暂不提交其他课程。
- 提交断网且未取得受理号时，结果可能未知，必须核对已选列表。

13:00:00 起先完成刷新，再按正常流程选课。刷新失败时先走异常恢复，不直接使用旧 Token 提交。
后续常规刷新仍按原配置执行，不因每条缓存提示重复整点刷新；网络超时、故障退避和 `Retry-After` 仍有效。
刷新本身有耗时，首个选课请求会晚于刷新开始时间；不保证服务端整点收到请求或提高成功率。
