...
- 12:59:59 前收到每日缓存提示时，等待至当天 **12:59:59** 再从第一顺位尝试，等待期间保留预热。此后仍提示缓存，按本次 `request_interval` 重试，不强制等待 800ms；设置 0.1 秒即等待 100ms。截止时间仍优先。
- 满额按间隔继续重试；网络或服务异常按 1、2、4、8、12 秒退避，并尊重 `Retry-After`。
- 已取得受理号时继续查询同一请求，结果未知时暂不提交其他课程。
- 提交断网且未取得受理号时，结果可能未知，必须核对已选列表。

12:59:59 起的缓存重试沿用本次间隔，但网络超时、故障退避和服务器 `Retry-After` 仍有效。间隔是响应处理后的等待，不保证固定请求频率或提高成功率。
